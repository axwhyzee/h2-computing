...
class Player(Entity):
    def __init__(self, x: int, y: int, symbol: str, lvl: int, premium: bool):
        ...
        """
        super().__init__(x, y, symbol, True)
        self.__lvl = lvl
...
class NonPremiumPlayer(Player):
    def __init__(self, x: int, y: int, symbol: str, lvl: int):
        ...
        super().__init__(x, y, symbol, lvl, False)
    ...
    def get_available_pokemons(self, pokemons: List[Pokemon]) -> List[Pokemon]:
        ...
        available = []
        center_x, center_y = self.get_pos()
        r = self.get_lvl()
        y_lower = center_y - r
        y_upper = center_y + r
        ...
        for p in pokemons:
            px, py = p.get_pos()
            if y_lower <= py <= y_upper:
                cir_x_left, cir_x_right = get_perimeter_x(py, r, center_x, center_y)
                if cir_x_left <= px <= cir_x_right:
                    available.append(p)        
        return available
...
class PremiumPlayer(Player):
    def __init__(self, x: int, y: int, symbol: str, lvl: int):
        ...
        super().__init__(x, y, symbol, lvl, True)
